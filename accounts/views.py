...
#         logger = logging.getLogger('command')
#         Uploadform  = CSVUploadForm
#         pail_data = model.objects.all().order_by('shipment_results')
#         serial_num = request.POST.get('serial_num')
#         pnum = model.objects.filter(shipment_results=serial_num).count()
#         # logger.info(serial_num)
#         # logger.info(model.objects.filter(delivery_record="").count())
#         # logger.info(model.objects.filter(shipment_results=serial_num).values_list('delivery_record',flat=True))
#         #連番を入力せずにボタンを押した場合
#         if (not request.POST.get('serial_num')) and (not request.POST.get('delivery_checker')):
#             message = '納品確認者と連番を入力してください'
#             d = {'Uploadform':Uploadform,'pail_data':pail_data,'message':message}
#             return render(request,'collation.html',d)
#         elif not request.POST.get('serial_num'):
#             message = '杭を読込んでください'
#             d = {'Uploadform':Uploadform,'pail_data':pail_data,'message':message}
#             return render(request,'collation.html',d)
#         elif not request.POST.get('delivery_checker'):
#             message = '納品確認者を入力してください'
#             d = {'Uploadform':Uploadform,'pail_data':pail_data,'message':message}
#             return render(request,'collation.html',d)
#         #入力値がDB内にないとき
#         if pnum == 0:
#             message = '読込んだ杭は出荷実績に含まれていません'
#             d = {'Uploadform':Uploadform,'pail_data':pail_data,
#                 # 'delivery_checker':request.POST.get('delivery_checker'),
#                 # 'serial_num':request.POST.get('serial_num'),
#                 'message':message
#             }
#             return render(request,'collation.html',d)
#         #＝＝＝＝＝＝＝＝＝＝＝＝＝出荷実績に対応した納品実績が埋まっていないとき（基本ここに入る）＝＝＝＝＝＝＝＝＝＝＝＝＝
#         # if model.objects.all().filter(shipment_results=serial_num,delivery_record__isnull=False):
#         # if not model.objects.filter(shipment_results=serial_num).values_list('delivery_record',flat=True):卓将が教えてくれたもの
#         if model.objects.filter(shipment_results=serial_num,delivery_record=""):
#             pail_num                 = model.objects.get(shipment_results=serial_num)
#             dt_now                   = timezone.datetime.now()
#             pail_num.delivery_record = dt_now.strftime('%Y-%m-%d %H:%M:%S')#納品実績に時刻を登録
#             pail_num.deliverer       = request.POST.get('delivery_checker')#納品確認者にテキストにある人名を登録
#             pail_num.save()
#             d = {'Uploadform':Uploadform,'pail_data':pail_data,}
#             return render(request,'collation.html',d)
#         else:
#             #出荷実績に対応した納品実績が埋まっているとき
#             message = '読込んだ杭は既に読込まれています'
#             d = {'Uploadform':Uploadform,'pail_data':pail_data,'message':message}
#             return render(request,'collation.html',d)
...
#         pnum = model.objects.filter(shipment_results=serial_num).count()
#         # if Pailnumform_con.is_valid():
#         # pail_num = Pailnumform_con.cleaned_data.get('serial_num')
#         # pnum = model.objects.filter(shipment_results=pail_num).count()
#         #連番を入力せずにボタンを押した場合
#         if (not request.POST.get('serial_num')) and (not request.POST.get('inspector')):
#             message = '検査者と連番を入力してください'
#             d = {'pail_data':pail_data,'message':message}
#             return render(request,'confirm.html',d)
#         elif not request.POST.get('serial_num'):
#             message = '杭を読込んでください'
#             d = {'pail_data':pail_data,'message':message}
#             return render(request,'confirm.html',d)
#         elif not request.POST.get('inspector'):
#             message = '検査者を入力してください'
#             d = {'pail_data':pail_data,'message':message}
#             return render(request,'confirm.html',d)
#         if pnum == 0:
#             message = '読込んだ杭は出荷実績に含まれていません'
#             d = {# 'Uploadform':Uploadform,
#                 'pail_data':pail_data,'message':message}
#             return render(request,'confirm.html',d)
#         #納品実績の時間が埋まっていない　→　納品確認を終えていないとき
#         if model.objects.filter(shipment_results=serial_num,delivery_record=""):
#             message = 'この杭は納品確認を終えていません\n納品確認を実施してください'
...
#         #＝＝＝＝＝＝＝＝＝＝＝＝＝出荷実績に対応した材料検査が埋まっていないとき（基本ここに入る）＝＝＝＝＝＝＝＝＝＝＝＝＝
#         # if model.objects.filter(shipment_results=pail_num, material_inspection__isnull=False):
#         if model.objects.filter(shipment_results=serial_num,material_inspection=""):
#             pail_num                     = model.objects.get(shipment_results=serial_num)
#             dt_now                       = timezone.datetime.now()
#             pail_num.material_inspection = dt_now.strftime('%Y-%m-%d %H:%M:%S')#材料検査に時刻を登録
#             pail_num.inspector           = request.POST.get('inspector')#検査者にテキストにある人名を登録
#             pail_num.save()
#             d = {
#                 'pail_data':pail_data,
#             }
#             return render(request,'confirm.html',d)
#         # Pailnumform_con.save()
#         else:
#             #＝＝＝＝＝＝＝＝＝＝＝＝＝出荷実績に対応した材料検査が埋まっているとき ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
#             message = '読込んだ杭は既に読込まれています'
#             d = {'pail_data':pail_data,'message':message,}
#             return render(request,'confirm.html',d)
#     else:
...
#         #＝＝＝＝＝＝＝＝＝＝＝＝＝出荷実績に対応した打設開始時刻が埋まっていないとき（基本ここに入る）＝＝＝＝＝＝＝＝＝＝＝＝＝
#         if model.objects.filter(shipment_results=serial_num, start_casting=""):
#             #打設者が入力されていないとき
#             if not request.POST.get('caster'):
#                 message = '打設者を入力してください'
#                 d = {'pail_data':pail_data,'message':message,}
#                 return render(request,'casting.html',d)
#             if not request.POST.get('pail_num'):
#                 message = '杭番号を入力してください'
...
#                 d = {'pail_data':pail_data,'message':message,}
#                 return render(request,'casting.html',d)
#             pail_num1                    = model.objects.get(shipment_results=serial_num)
#             dt_now                       = timezone.datetime.now()
#             pail_num1.start_casting      = dt_now.strftime('%Y-%m-%d %H:%M:%S')#材料検査に時刻を登録
#             pail_num1.caster             = request.POST.get('caster')#打設者にテキストにある人名を登録
#             pail_num1.pail_num           = request.POST.get('pail_num')#杭番号に登録
#             pail_num1.cross_section      = request.POST.get('cross_section')#断面に登録
#             pail_num1.save()
#             d = {
#                 'pail_data':pail_data,
#             }
